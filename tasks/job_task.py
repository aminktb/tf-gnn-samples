...
CitationData = namedtuple(
    "CitationData", ["adj_lists", "num_incoming_edges", "features", "labels", "mask"]
)


class Job_Task(Sparse_Graph_Task):
    @classmethod
    ...
    def __load_data(self, data_directory: RichPath):
        assert isinstance(
            data_directory, LocalPath
        ), "CitationNetworkTask can only handle local data"
        ...
        train_data = [
            self.__preprocess_data(
                adj_list, features, np.argmax(train_labels, axis=1), train_mask
            )
        ]
        valid_data = [
            self.__preprocess_data(
                adj_list, features, np.argmax(valid_labels, axis=1), valid_mask
            )
        ]
        test_data = [
            self.__preprocess_data(
                adj_list, features, np.argmax(test_labels, axis=1), test_mask
            )
        ]
        return train_data, valid_data, test_data

    def __preprocess_data(
        self, adj_list: Dict[int, List[int]], features, labels, mask
    ) -> CitationData:
        flat_adj_list = []
        self_loop_adj_list = []
        ...
        num_incoming_edges = np.stack(
            [np.ones_like(num_incoming_edges, dtype=np.int32), num_incoming_edges]
        )  # Shape [2, V]
        return CitationData(
            adj_lists=[self_loop_adj_list, flat_adj_list],
    ...
    def make_task_output_model(
        ...
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=output_label_logits, labels=placeholders["labels"]
        )
        total_loss = tf.reduce_sum(losses * placeholders["mask"])

        correct_preds = tf.equal(
            tf.argmax(output_label_logits, axis=1, output_type=tf.int32),
            placeholders["labels"],
        )
        num_masked_correct = tf.reduce_sum(
            tf.cast(correct_preds, tf.float32) * placeholders["mask"]
        )
